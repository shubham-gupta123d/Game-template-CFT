...
function Lotto() {
  ...
  const selectedBanner = useSelector((state) => state.banner.selectedBanner);
  ...
  // get numeric ticket price from a string like "₹75" or "₹5,00,00,000"
  const parsePrice = (priceStr) => {
    if (!priceStr) return 40;
    const digits = priceStr.toString().replace(/[^\d]/g, '');
    return digits ? parseInt(digits, 10) : 40;
  };
  ...
  const handleAddToCart = () => {
    ...
    if (!selectedBanner) {
      toast.error('No draw/banner selected. Use the slider to pick a draw.');
      return;
    }
  ...
  return (
    <div className="bg-black min-h-screen flex flex-col items-center py-8 px-4 font-poppins">
      ...
      <h1 className="text-3xl md:text-5xl font-bold text-[#D4AC54] mb-6">
        {selectedBanner?.title || 'Royal Lotto'}
      </h1>
      ...
      <div className="mt-6 rounded-2xl w-full max-w-5xl bg-white border border-black shadow-lg">
        ...
        <div className="flex justify-between items-center mt-6 px-6 pb-6">
          <button className="bg-[#D4AC54] text-black font-semibold px-6 py-2 rounded-lg shadow border border-black hover:bg-yellow-600">
            Entry ₹{selectedBanner ? parsePrice(selectedBanner.price) : 40}
          </button>
          <button
